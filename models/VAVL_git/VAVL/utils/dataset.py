...
class AudVidSet(torch_utils.data.Dataset):
    def __init__(self, *args, **kwargs):
        ...
        self.print_utt = kwargs.get("print_utt", False)
        ...
        if self.lab_type == "categorical":
            ...
            assert (
                self.label_config.get("max_score", None) != None and self.label_config.get("min_score", None) != None
    ...
    def __getitem__(self, idx):
        ...
        if self.print_dur:
            result = (cur_wav, cur_vid, cur_lab, cur_dur, cur_utt)
        if self.print_utt:
            result = (cur_wav, cur_vid, cur_lab, cur_dur, cur_utt)
            # print('hello')
        return result
...
def collate_fn_padd(batch):
    ...
    return total_wav, total_vid, total_lab, attention_mask, total_utt
