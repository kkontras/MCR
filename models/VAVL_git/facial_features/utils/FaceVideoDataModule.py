...
from models.VAVL_git.facial_features.utils.FaceDataModuleBase import FaceDataModuleBase
from models.VAVL_git.facial_features.utils.ImageDatasetHelpers import bbpoint_warp, point2bbox
from models.VAVL_git.facial_features.utils.ImageTestDataset import TestData
from models.VAVL_git.facial_features.utils.UnsupervisedImageDataset import UnsupervisedImageDataset

# from ffprobe imp ort FFProbe
...
class FaceVideoDataModule(FaceDataModuleBase):
    ...
    def _unpack_video(self, video_idx, overwrite=False):
        ...
        if n_frames == expected_frames:
            ...
            print("[WARNING] Expected %d frames but got %d vor video '%s'" % (expected_frames, n_frames, str(video_file)))
    ...
    def _detect_faces_in_sequence(self, sequence_id):
        ...
        for fid, frame_fname in enumerate(tqdm(range(start_fid, len(frame_list)))):
            ...
            self._detect_faces_in_image_wrapper(
                frame_list,
                fid,
                out_detection_folder,
                out_landmark_folder,
                out_file,
                centers_all,
                sizes_all,
        ...
        FaceVideoDataModule.save_detections(
            out_file,
            detection_fnames_all,
            landmark_fnames_all,
            centers_all,
    ...
    def _gather_video_metadata(self):
        ...
        for vi, vid_file in enumerate(tqdm(self.video_list)):
            path_file = str(os.path.join(Path(self.root_dir), vid_file))
            print(path_file)
            try:
                # vid = FFProbe(filename=path_file)
                vid = ffmpeg.probe(filename=path_file)
            ...
            # print(vid['streams'], '0')
            # print(vid['streams'][2], '1')
            # print(vid['streams'][2]['codec_type'] == 'video', '2')
            ...
            # codec_idx = 2 # codec_idx[0]
            print(codec_idx)
            vid_info = vid["streams"][codec_idx[0]]
            # print(vid)
            # print(vid['streams'][0], 'haha')
            vid_meta = {}
            ...
            if str(vid_file)[-3:] == "flv":
                ...
                r_frame_rate = vid_info["r_frame_rate"]
                num, den = map(int, r_frame_rate.split("/"))  # Split the frame rate into numerator and denominator
                frame_rate = num / den

                vid_meta["num_frames"] = int(duration * frame_rate)
    ...
    def _get_detection_for_sequence(self, sid):
        ...
        if not out_file.exists():
            ...
            return [], [], [], 0
        detection_fnames, landmark_fnames, centers, sizes, last_frame_id = FaceVideoDataModule.load_detections(out_file)
    ...
    def create_reconstruction_video(
        ...
        vis_fnames = self._get_reconstructions_for_sequence(
            sequence_id,
            rec_method=rec_method,
            retarget_suffix=retarget_suffix,
        ...
        if outfile.exists() and not overwrite:
            ...
            attach_audio_to_reconstruction_video(
                outfile,
                self.root_dir / self.video_list[sequence_id],
                overwrite=overwrite,
            )
            return
        ...
        for fid in tqdm(range(len(vid_frames))):
            ...
            if writer is None:
                ...
                writer = cv2.VideoWriter(
                    ...
                    cv2.CAP_FFMPEG,
                    fourcc,
                    int(self.video_metas[sequence_id]["fps"].split("/")[0]),
                    (im.shape[1], im.shape[0]),
                    True,
                )
            im_cv = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
    ...
    def _draw_annotation(self, frame_draw: ImageDraw.Draw, val_gt: dict, font, color):
        pass

    def create_reconstruction_video_with_recognition_and_annotations(self, sequence_id, overwrite=False, distance_threshold=None):
        ...
        (
            validated_detection_fnames,
            validated_annotations,
            validated_recognition_labels,
            discarded_annotations,
            detection_not_found,
    ...
    def _gather_detections_for_sequence(self, sequence_id, with_recognitions=True):
        ...
        if out_file_detections.exists() and (not with_recognitions or out_file_recognitions.exists()):
            detection_fnames, landmark_fnames, centers, sizes, last_frame_id = FaceVideoDataModule.load_detections(out_file_detections)
            if with_recognitions:
    ...
    def _identify_recognitions_for_sequence(self, sequence_id, distance_threshold=None):
        ...
        for label in counter.keys():
            ...
            try:
                recognized_filenames_label = sorted([recognized_detections_fnames[i].relative_to(self.output_dir) for i in indices.tolist()])
            except ValueError:
                recognized_filenames_label = sorted([recognized_detections_fnames[i].relative_to(recognized_detections_fnames[i].parents[4]) for i in indices.tolist()])
        ...
        FaceVideoDataModule._save_recognitions(
            out_file,
            labels,
            recognition_indices,
            recognition_means,
    ...
    def _gather_detections(self, with_recognitions=True):
        ...
        for sid in range(self.num_sequences):
            ...
            if with_recognitions:
                ...
                detection_fnames, centers, sizes = self._gather_detections_for_sequence(sid, with_recognitions=with_recognitions)
                embeddings = None
    ...
    def _map_detections_to_gt(
        self,
        detection_filenames: list,
        annotation_file,
        annotation_type,
        # specifier = ''
    ):
        ...
        for i in range(annotation.shape[0]):
            ...
            if len(detections) > 1:
                # if specifier == '':
                print(f"Frame {frame_idx} has {len(detections)} detections of the equally " f"classified identity. Only one will be taken")
        ...
        print(
            f"For {total_annotated_frames} annotations, "
    ...
    def assign_gt_to_detections_sequence(self, sequence_id):
        ...
        def second_most_frequent_label():
            if len(most_frequent_labels) == 2:
                ...
                raise RuntimeError("Too many labels occurred with the same frequency. Unclear which one to pick.")
            else:
        ...
        #      annotations]
        suffs = [str(anno.stem)[len(str(self.video_list[sequence_id].stem)) :] for anno in annotations]
        ...
        if len(annotations) == 1 and suffs[0] == "":
            ...
            if len(most_frequent_labels) != 1:
                raise ValueError("There seem to be two people at the same time in all pictures but we only " "have annotation for one")
            ...
            (
                main_valid_detection_list,
                main_valid_annotation_list,
                main_discarded_list,
                main_detection_not_found_list,
            ...
            detection_fnames, detection_centers, detection_sizes, _ = self._get_detection_for_sequence(sequence_id)
            ...
            (
                other_valid_detection_list,
                other_valid_annotation_list,
                other_discarded_list,
                other_detection_not_found_list,
            ) = self._map_detections_to_gt(other_detection_file_names, other_annotation_file, anno_type)

            other_center = self._get_bb_center_from_fname(other_detection_file_names[0], detection_fnames, detection_centers)
            main_center = self._get_bb_center_from_fname(main_detection_file_names[0], detection_fnames, detection_centers)
            ...
            if suffs[0] == "" and (suffs[1] == "_left" or suffs[1] == "_right"):
                ...
                if suffs[1] == "_left":
                    ...
                    if correct_left_right_order(main_center, other_center) != 1:
                        raise RuntimeError("The main detection should be on the left and the other on the right but this is not the case")
            ...
            # THE CASE OF TWO ROUGHLY EQUALY DOMINANT DETECTIONS (LEFT and RIGHT)
                ...
                if order == -1:
                    ...
                    raise RuntimeError("Left and right detections have centers in the same place. " "No way to tell left from right")

                left_detection_file_names = recognition_fnames[left_label]
                left_annotation_file = annotations[0]
                (
                    left_valid_detection_list,
                    left_annotation_list,
                    left_discarded_list,
                    left_detection_not_found_list,
                ...
                (
                    right_valid_detection_list,
                    right_valid_annotation_list,
                    right_discarded_list,
                    right_detection_not_found_list,
                ...
                print("Unexpected annotation case found.")
                print(suffs)
                raise RuntimeError(f"Unexpected annotation case found: {str(suffs)}")

        out_folder = self._get_path_to_sequence_detections(sequence_id)
        out_file = out_folder / "valid_annotations.pkl"
        FaceVideoDataModule._save_annotations(
            out_file,
            annotated_detection_fnames,
            validated_annotations,
            recognition_label_dict,
            discarded_annotations,
            detection_not_found,
        )
...
class TestFaceVideoDM(FaceVideoDataModule):
    ...
    def _get_reconstructions_for_sequence(self, sid, rec_method="emoca", retarget_suffix=None, image_type=None):
        out_folder = self._get_path_to_sequence_results(sid, rec_method=rec_method, suffix=retarget_suffix)
