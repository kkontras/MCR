...
class CategoryBoxEmbeddings(nn.Module):
    def __init__(self):
        ...
        # [Hand, Object, Padding]
        self.category_embeddings = nn.Embedding(embedding_dim=self.hidden_size, num_embeddings=3, padding_idx=0)
        self.box_embedding = nn.Linear(4, self.hidden_size)
        self.score_embedding = nn.Linear(1, self.hidden_size)
        # [Pad, Left, Right]
        self.side_embedding = nn.Embedding(num_embeddings=3, embedding_dim=self.hidden_size, padding_idx=0)
        # [Pad, No contact, Self contact, Another person, Portable obj., Stationary obj.]
        self.state_embedding = nn.Embedding(num_embeddings=6, embedding_dim=self.hidden_size, padding_idx=0)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        embeddings = category_embeddings + boxes_embeddings + score_embeddings + sides_embedding + states_embedding
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        # Add CLS token
        cb_embeddings = torch.cat((self.cls_token.expand(bs, nf, -1, -1), cb_embeddings), dim=2)
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        position_embeddings = self.position_embeddings(torch.arange(nf, device=layout_embeddings.device).expand(1, -1))
        ...
        # [Num. frames, Batch size, Hidden size]
        causal_mask = generate_square_subsequent_mask(embeddings.size(0)).to(embeddings.device)
...
class Stlt(nn.Module):
    def __init__(self, encs, args: CfgNode, checkpoint: str = None, frozen_encoders: bool = False):
        ...
        # Rest
        self.temporal_transformer = TemporalTransformer(self.args)
        # Build classifier
        # self.classifiers = nn.Linear(self.hidden_size, self.args.num_classes)
        self.classifiers = nn.ModuleDict({"ACTION": nn.Linear(self.hidden_size, self.args.num_classes)})
        ...
        if self.frozen_encoders:
            self.temporal_transformer.requires_grad_(False)
