...
from unimodals.common_models import Identity, Reshape, Sequential, Transpose
...
haptics_robust_trajectories = Task.get_test_trajectories(haptics_noise=True, **dataset_args)
controls_robust_trajectories = Task.get_test_trajectories(controls_noise=True, **dataset_args)
multimodal_robust_trajectories = Task.get_test_trajectories(multimodal_noise=True, **dataset_args)
...
for i in range(len(multimodal_robust_trajectories)):
    multimodal_robust_loader.append(
        DataLoader(
...
encoders = [
    Sequential(
        Transpose(0, 1),
        layers.observation_pos_layers(64),
        GentlePushLateLSTM(64, 256),
        Transpose(0, 1),
    ),
    Sequential(
        Transpose(0, 1),
        layers.observation_sensors_layers(64),
        GentlePushLateLSTM(64, 256),
        Transpose(0, 1),
    ),
    Sequential(
...
filename = general_train(trainprocess, "gentle_push_LF")
