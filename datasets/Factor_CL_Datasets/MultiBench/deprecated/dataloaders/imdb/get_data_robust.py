...
class IMDBDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        image = self.dataset["images"][ind + self.start_ind] if not self.vggfeature else self.dataset["vgg_features"][ind + self.start_ind]
...
def get_dataloader_robust(
    ...
    clsf = VGGClassifier(
        model_path="/home/pliang/multibench/MultiBench/datasets/imdb/vgg16.tar",
        synset_words="synset_words.txt",
    )
    googleword2vec = KeyedVectors.load_word2vec_format(
        "/home/pliang/multibench/MultiBench/datasets/imdb/GoogleNews-vectors-negative300.bin.gz",
        binary=True,
    )
    ...
    for noise_level in range(11):
        ...
        if extract_vgg:
            vgg_features = []
            images_robust = add_visual_noise(images, noise_level=noise_level / 10)
    ...
    for test in robust_vision:
        robust_vision_dataloader.append(
            DataLoader(
                IMDBDataset_robust(test, 0, len(test)),
                shuffle=False,
                num_workers=num_workers,
                batch_size=batch_size,
            )
