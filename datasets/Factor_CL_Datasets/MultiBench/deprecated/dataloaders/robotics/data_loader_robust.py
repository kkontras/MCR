...
def get_data(device, configs, filedirprefix="", unimodal=None, output="contact_next"):
    ...
    print("Number of files in multifile dataset = {}".format(len(filename_list)))

    val_filename_list = []

    val_index = np.random.randint(0, len(filename_list), int(len(filename_list) * configs["val_ratio"]))
    ...
    val_filename_list1, filename_list1 = augment_val(val_filename_list, filename_list)
    ...
    samplers["train"] = SubsetRandomSampler(range(len(filename_list1) * (configs["ep_length"] - 1)))
    ...
    dataloaders["test"] = []
    if image_noise:
        dataloaders["test"].append(dl_image)
    if prop_noise:
        dataloaders["test"].append(dl_prop)
    if force_noise:
        dataloaders["test"].append(dl_force)
