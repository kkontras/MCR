...
def get_dataloader(
    ...
    train_dataset, val_dataset, test_dataset = random_split(
        dataset,
        [size // 10, size // 10, size - 2 * (size // 10)],
...
class Affectdataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.data_kind == "pkl":
            context_num = len(self.dataset["words"][item_index]["context_sentences"])
            for modal in self.modals:
                if modal == "covarep" or "open_face":
                    features.append(torch.tensor(self.dataset[modal][item_index]["punchline_features"]))
                    features.append([torch.tensor(self.dataset[modal][item_index]["context_features"][i]) for i in range(context_num)])
                elif modal == "words":
                    punchline_embedding = []
                    for embedding_ind in self.dataset[modal][item_index]["punchline_embedding_indexes"]:
                        punchline_embedding.append(self.word_embedding[embedding_ind])
                    ...
                    for context_ind in range(context_num):
                        ...
                        for embedding_ind in self.dataset[modal][item_index]["context_embedding_indexes"][context_ind]:
                            context_embedding.append(self.word_embedding[embedding_ind])
                        context_embeddings.append(torch.tensor(context_embedding))
            ...
            for modal in self.modals:
                if "label" not in modal.lower() and modal != "words":
                    ...
                    features.append(self.dataset[modal][item_index]["features"])
                    # TO DO: can use pretrained word embedding
                else:
