...
class Affectdataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        label = torch.tensor(self.dataset["labels"][ind]).round().long() + 3 if self.task == "classification" else torch.tensor(self.dataset["labels"][ind]).float()
        if self.flatten:
...
def get_dataloader(
    ...
    train = DataLoader(
        Affectdataset(alldata["train"], flatten_time_series, task=task),
    ...
    valid = DataLoader(
        Affectdataset(alldata["valid"], flatten_time_series, task=task),
        shuffle=False,
        num_workers=num_workers,
        batch_size=batch_size,
        collate_fn=process,
    ...
    for i in range(10):
        ...
        robust_text.append(
            DataLoader(
                Affectdataset(test, flatten_time_series, task=task),
                shuffle=False,
                num_workers=num_workers,
                batch_size=batch_size,
                collate_fn=process,
    ...
    for i in range(10):
        test = dict()
        test["vision"] = alldata["test"]["vision"]
        test["audio"] = add_timeseries_noise([alldata["test"]["audio"]], noise_level=i / 10, rand_drop=False)
        test["text"] = alldata["test"]["text"]
        test["labels"] = alldata["test"]["label"]
        test = drop_entry(test)
        robust_audio.append(
    ...
    for i, text in enumerate(robust_text):
        alldata_test = add_timeseries_noise(
            [alldata["test"]["vision"], alldata["test"]["audio"], text],
            noise_level=i / 10,
        )
...
def process(inputs: List):
    ...
    return (
        ...
        torch.tensor(labels).view(len(inputs)),
    )
