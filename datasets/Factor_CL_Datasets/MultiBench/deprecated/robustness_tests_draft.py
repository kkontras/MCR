...
def periodic(img, periodic_noise_filename="periodic_noise"):
    ...
    for i in range(6):
        ...
        noise = random_crop(
            rotate(
                noise.resize((width * 2, height * 2)),
                np.random.random_sample() * 360,
                "white",
            ),
            height,
            width,
...
def audio_robustness(tests, noise_level=0.3, noises=None):
    if noises == None:
        noises = [
            ...
            audio_random_dropout,
            audio_structured_dropout,
        ]
