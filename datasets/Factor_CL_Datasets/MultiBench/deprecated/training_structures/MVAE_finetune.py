...
def train_MVAE(
    ...
    finetune_epoch,
    finetune_every,
    backbone_optimtype=torch.optim.Adam,
    finetune_optimtype=torch.optim.SGD,
    finetune_batch_size=40,
    learning_rate=0.001,
    savedirbackbone="best1.pt",
    savedirhead="best2.pt",
):
        ...
        for j in train_dataloader:
            ...
            for i in range(len(trains)):
                total_loss += backbone_objective(
                    allnonebuti(i, recons[i]),
                    allnonebuti(i, trains[i]),
                    mus[i],
                    vars[i],
                )
            total_loss.backward()
