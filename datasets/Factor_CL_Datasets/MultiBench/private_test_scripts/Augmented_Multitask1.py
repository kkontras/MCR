    ...
    for epoch in range(total_epochs):
        ...
        for j in train_dataloader:
            ...
            if is_packed:
                with torch.backends.cudnn.flags(enabled=False):
                    out = model(
                        [
                            [i.to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for i in j[0]],
                            j[1],
                        ]
                    )
                ...
                out1, out2 = model([i.float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for i in j[:-2]])

                loss1 = criterion(
                    out1,
                    j[-2].long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
                loss2 = criterion(
                    out2,
                    j[-1].long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
                # loss = loss1+loss2
                loss = loss1
        ...
        with torch.no_grad():
            ...
            for j in valid_dataloader:
                ...
                loss1 = criterion(
                    out1,
                    l1.long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
                loss2 = criterion(
                    out2,
                    l2.long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
                totalloss += loss * len(j[-1])
        ...
        if task == "classification":
            ...
            print("Epoch " + str(epoch) + " valid loss: " + str(valloss) + " acc1: " + str(acc1) + " acc2: " + str(acc2))
            if acc1 > bestacc:
                patience = 0
                bestacc = acc1
                print("Saving Best")
...
def test(
    ...
    with torch.no_grad():
        ...
        for j in test_dataloader:
            ...
            loss1 = criterion(
                out1,
                l1.to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
            )
            loss2 = criterion(
                out2,
                l2.to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
            )
