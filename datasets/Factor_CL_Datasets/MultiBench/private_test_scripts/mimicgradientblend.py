...
encoders = [
    MLP(5, 10, 10).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    GRU(12, 30, flatten=True).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
]
...
unimodal_heads = [
    MLP(10, 20, 6).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    MLP(720, 40, 6).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
]
allmodules = [
    encoders[0],
    encoders[1],
