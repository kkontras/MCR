...
decoders = [
    MLP(n_latent, 600, 300).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    MLP(n_latent, 2048, 4096).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
]
intermediates = [
...
all_in_one_test(testprocess, [encoders[0], encoders[1], fusion, head, intermediates[-1]])
