...
class Regularization(object):
    ...
    def get_batch_statistics(cls, loss: torch.Tensor, n_samples: int, estimation: str = "ent") -> torch.Tensor:
        ...
        if estimation == "var":
            ...
            raise NotImplementedError(f'{estimation} is unknown regularization, please use "var" or "ent".')  # pragma: no cover
    ...
    @classmethod
    def get_regularization_term(cls, inf_scores: torch.Tensor, norm: float = 2.0, optim_method: str = "max_ent") -> torch.Tensor:
        """
    ...
    def forward(self, logits, inputs):
        ...
        if self.pack:
            ...
            for ind, i in enumerate(inputs[0]):
                inf_inputs.append(Perturbation.perturb_tensor(i, self.reg_params.n_samples).float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")))
                inf_inputs_len.append(Perturbation.perturb_tensor(inputs[1][ind], self.reg_params.n_samples, False))
            self.model.train()
            inf_output = self.model([inf_inputs, inf_inputs_len])
        else:
        ...
        grads = [Regularization.get_batch_norm(gradients[k], loss=inf_loss, estimation=self.reg_params.estimation) for k in range(2)]

        inf_scores = torch.stack(grads)
        reg_term = Regularization.get_regularization_term(
            inf_scores,
            norm=self.reg_params.norm,
