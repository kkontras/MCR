...
from objective_functions.recon import elbo_loss, recon_weighted_sum
...
def _criterioning(pred, truth, criterion):
    ...
    if isinstance(criterion, torch.nn.CrossEntropyLoss):
        ...
        return criterion(
            pred,
            truth.long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        )
    if isinstance(
        criterion,
        ...
        return criterion(
            pred,
            truth.float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        )
...
def MVAE_objective(
    ...
    def _actualfunc(pred, truth, args):
        ...
        for i in range(len(inps)):
            reconsjoint.append(decoders[i](_reparameterize(fusedmu, fusedlogvar, training)))
        total_loss = recon_loss_func(reconsjoint, inputs, fusedmu, fusedlogvar)
        for i in range(len(inps)):
            mu, logvar = reps[i]
            recon = decoders[i](_reparameterize(mu, logvar, training))
            total_loss += recon_loss_func(_allnonebuti(i, recon), _allnonebuti(i, inputs[i]), mu, logvar)
...
def RefNet_objective(ref_weight, criterion=torch.nn.CrossEntropyLoss(), input_to_float=True):
    ...
    def _actualfunc(pred, truth, args):
        ...
        if input_to_float:
            inputs = [torch.flatten(t, start_dim=1).float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for t in inps]
        else:
        ...
        for i in range(len(inps)):
            ...
            loc += inputsizes[i]
            ss_loss += ss_criterion(
                out,
                inputs[i],
                torch.ones(out.size(0)).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
            )
...
def RMFE_object(reg_weight=1e-10, criterion=torch.nn.BCEWithLogitsLoss(), is_packed=False):
    ...
    def _regfunc(pred, truth, args):
        ...
        try:
            reg_loss = lossfunc(
                pred,
                [i.to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for i in inps],
            )
        except RuntimeError:
