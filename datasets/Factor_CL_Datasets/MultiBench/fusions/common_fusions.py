...
class MultiplicativeInteractions3Modal(nn.Module):
    ...
    def __init__(self, input_dims, output_dim, task=None):
        ...
        self.a = MultiplicativeInteractions2Modal([input_dims[0], input_dims[1]], [input_dims[2], output_dim], "matrix3D")
        self.b = MultiplicativeInteractions2Modal([input_dims[0], input_dims[1]], output_dim, "matrix")
        self.task = task
...
class MultiplicativeInteractions2Modal(nn.Module):
    ...
    def forward(self, modalities):
        ...
        # Scales and Biases.
            ...
            output = self._repeatHorizontally(Wprime, self.input_dims[1]) * m2 + self._repeatHorizontally(bprime, self.input_dims[1])
        return output
...
class TensorFusion(nn.Module):
    ...
    def forward(self, modalities):
        ...
        m = torch.cat(
            (
                Variable(
                    torch.ones(*nonfeature_size, 1).type(mod0.dtype).to(mod0.device),
                    requires_grad=False,
                ),
                mod0,
            ),
        ...
        for mod in modalities[1:]:
            mod = torch.cat(
                (
                    Variable(
                        torch.ones(*nonfeature_size, 1).type(mod.dtype).to(mod.device),
                        requires_grad=False,
                    ),
                    mod,
                ),
...
class LowRankTensorFusion(nn.Module):
    ...
    def __init__(self, input_dims, output_dim, rank, flatten=True):
        ...
        for input_dim in input_dims:
            factor = nn.Parameter(torch.Tensor(self.rank, input_dim + 1, self.output_dim)).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
        ...
        self.fusion_bias = nn.Parameter(torch.Tensor(1, self.output_dim)).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
    ...
    def forward(self, modalities):
        ...
        for modality, factor in zip(modalities, self.factors):
            ones = Variable(torch.ones(batch_size, 1).type(modality.dtype), requires_grad=False).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
            if self.flatten:
