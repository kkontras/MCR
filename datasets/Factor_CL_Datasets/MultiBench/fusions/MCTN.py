...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, n_layers=1, dropout=0.2):
        ...
        self.gru = nn.GRU(hidden_size + output_size, hidden_size, n_layers, dropout=dropout)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        outputs = Variable(torch.zeros(max_len, batch_size, output_size)).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))

        encoder_output, hidden = self.encoder(src)
        hidden = hidden[: self.decoder.n_layers]

        if self.training:
            output = Variable(torch.zeros_like(trg.data[0, :]))  # solve the bug of input.size must be equal to input_size
        else:
