...
def get_dataloader(
    ...
    val_ds = _MyDataset(X[val_split:test_split], Y[val_split:test_split], modality_first)
    ...
    for noise_level in range(9):
        ...
        X_robust = torch.tensor(
            add_timeseries_noise(X_robust, noise_level=noise_level / 10),
            dtype=torch.float32,
        )
        if cuda:
        ...
        test_loader["timeseries"].append(torch.utils.data.DataLoader(test_ds, shuffle=False, batch_size=batch_size, drop_last=False))
