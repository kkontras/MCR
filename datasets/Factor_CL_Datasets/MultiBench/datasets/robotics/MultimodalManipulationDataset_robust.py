...
class MultimodalManipulationDataset_robust(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if dataset_index >= self.episode_length - self.n_time_steps - 1:
            dataset_index = np.random.randint(self.episode_length - self.n_time_steps - 1)
    ...
    def _get_single(self, dataset_name, list_index, unpaired_filename, dataset_index, unpaired_idx):
        ...
        if self.training_type == "selfsupervised":
            ...
            if self.image_noise:
                image = add_visual_noise([image], noise_level=self.noise_level)[0]
            depth = dataset["depth_data"][dataset_index]
            proprio = dataset["proprio"][dataset_index][:8]
            if self.prop_noise:
                proprio = add_timeseries_noise([proprio], noise_level=self.noise_level)[0]
            force = dataset["ee_forces_continuous"][dataset_index]
            if self.force_noise:
                force = add_timeseries_noise([force], noise_level=self.noise_level)[0]
            ...
            sample = {
                ...
                "contact_next": np.array([dataset["contact"][dataset_index + 1].sum() > 0]).astype(np.float),
    ...
    def _config_checks(self):
        if self.training_type != "selfsupervised":
            raise ValueError("Training type not supported: {}".format(self.training_type))
