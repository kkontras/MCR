...
class EnricoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.img_noise:
            screenImg = Image.fromarray(add_visual_noise([np.array(screenImg)], noise_level=self.noise_level)[0])
        ...
        if self.wireframe_noise:
            screenWireframeImg = Image.fromarray(add_visual_noise([np.array(screenWireframeImg)], noise_level=self.noise_level)[0])
...
def get_dataloader(
    ...
    dl_test["image"] = [DataLoader(test, shuffle=False, num_workers=num_workers, batch_size=batch_size) for test in ds_test_img]
    # Add wireframe image noise
    for i in range(11):
        ds_test_wireframe.append(EnricoDataset(data_dir, wireframe_noise=True, noise_level=i / 10))
    dl_test["wireframe image"] = [DataLoader(test, shuffle=False, num_workers=num_workers, batch_size=batch_size) for test in ds_test_wireframe]
