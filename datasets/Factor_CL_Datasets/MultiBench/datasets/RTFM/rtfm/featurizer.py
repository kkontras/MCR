...
class Text(Featurizer):
    ...
    def featurize(self, task, eos="pad", pad="pad"):
        ...
        for y in range(0, len(mat)):
            ...
            smat.append(srow)
            lmat.append(lrow)
        wiki, wiki_length = self.lookup_sentence(
            (task.get_tokenized_wiki() if hasattr(task, "get_tokenized_wiki") else task.get_wiki()),
            task.vocab,
            max_len=task.max_wiki,
            eos=eos,
            pad=pad,
        )
        ins, ins_length = self.lookup_sentence(
            (task.get_tokenized_task() if hasattr(task, "get_tokenized_task") else task.get_task()),
            task.vocab,
            max_len=task.max_task,
            eos=eos,
            pad=pad,
        )
