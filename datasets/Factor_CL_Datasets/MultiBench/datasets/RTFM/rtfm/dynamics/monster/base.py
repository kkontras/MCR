...
class BaseMonster(O.WorldObject):
    ...
    def calculate_attack_success(cls, attacker, victim, engine=None):
        ...
        if engine is not None:
            engine.queue_immediate_event(
                E.Log("{} rolled a combat roll of {} for {} to_hit of {} ({}% hit rate)".format(attacker, roll, to_hit, victim, to_hit / 20 * 100))
            )
        return roll < to_hit
