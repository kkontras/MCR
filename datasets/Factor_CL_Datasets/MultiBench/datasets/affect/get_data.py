...
from datasets.Factor_CL_Datasets.MultiBench.robustness.text_robust import add_text_noise
from datasets.Factor_CL_Datasets.MultiBench.robustness.timeseries_robust import add_timeseries_noise
...
def z_norm(dataset, max_seq_len=50):
    ...
    for ind in range(dataset["text"].shape[0]):
        vision[ind] = np.nan_to_num((vision[ind] - vision[ind].mean(0, keepdims=True)) / (np.std(vision[ind], axis=0, keepdims=True)))
        audio[ind] = np.nan_to_num((audio[ind] - audio[ind].mean(0, keepdims=True)) / (np.std(audio[ind], axis=0, keepdims=True)))
        text[ind] = np.nan_to_num((text[ind] - text[ind].mean(0, keepdims=True)) / (np.std(text[ind], axis=0, keepdims=True)))
...
def get_dataloader(
    ...
    robust_test=False,
    data_type="mosi",
    raw_path="/home/van/backup/pack/mosi/mosi.hdf5",
    z_norm=False,
) -> DataLoader:
