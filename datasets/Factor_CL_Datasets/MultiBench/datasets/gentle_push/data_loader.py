...
class PushTask:
    ...
    def get_dataloader(
        ...
        **dataset_args,
        ...
        test_trajectories = cls.get_test_trajectories(modalities, **dataset_args)
        train_loader = DataLoader(
            SubsequenceDataset(train_trajectories, subsequence_length, modalities),
            batch_size=batch_size,
            shuffle=True,
            drop_last=drop_last,
        )
        val_loader = DataLoader(
            SubsequenceDataset(val_trajectories, subsequence_length, modalities),
            batch_size=batch_size,
            shuffle=True,
        )
        test_loader = dict()
    ...
    @classmethod
    def get_train_trajectories(cls, **dataset_args) -> List[TrajectoryNumpy]:
        """Get training trajectories."""
        kloss_dataset = dataset_args["kloss_dataset"] if "kloss_dataset" in dataset_args else False
        if kloss_dataset:
    ...
    @classmethod
    def get_eval_trajectories(cls, **dataset_args) -> List[TrajectoryNumpy]:
    ...
    @classmethod
    def get_test_trajectories(cls, modalities, **dataset_args):
        ...
        if kloss_dataset:
            ...
            if modalities == None or "image" in modalities:
                trajectories["image"] = []
                for i in range(10):
                    trajectories["image"].append(_load_trajectories("gentle_push_300.hdf5", visual_noise=i / 10, **dataset_args))
            if modalities == None or "gripper_pos" in modalities:
                trajectories["proprio"] = []
                for i in range(10):
                    trajectories["proprio"].append(_load_trajectories("gentle_push_300.hdf5", prop_noise=i / 10, **dataset_args))
            if modalities == None or "gripper_sensors" in modalities:
                trajectories["haptics"] = []
                for i in range(10):
                    trajectories["haptics"].append(_load_trajectories("gentle_push_300.hdf5", haptics_noise=i / 10, **dataset_args))
            if modalities == None or "controls" in modalities:
                trajectories["controls"] = []
                for i in range(10):
                    trajectories["controls"].append(
                        _load_trajectories(
            ...
            if modalities == None:
                ...
                for i in range(10):
                    trajectories["multimodal"].append(
                        _load_trajectories(
                            "gentle_push_300.hdf5",
                            multimodal_noise=i / 10,
                            **dataset_args,
                        )
                    )
            return trajectories
...
def _load_trajectories(
    *input_files,
    ...
    for name in input_files:
        ...
        with fannypack.data.TrajectoriesFile(fannypack.data.cached_drive_file(name, dataset_urls[name])) as f:
        ...
        for raw_trajectory_index, raw_trajectory in enumerate(raw_trajectories):
            ...
            if visual_noise != 0:
                observations["image"] = np.array(add_visual_noise(observations["image"], noise_level=visual_noise))
            ...
            if not use_vision:
                ...
                # Apply blackout rate
                image_mask = (np.random.uniform(size=(timesteps,)) > image_blackout_ratio).astype(np.float32).reshape((timesteps, 1, 1))
            ...
            np.concatenate(
                [
                    ...
                    raw_trajectory["contact"][:, np.newaxis],  # "contact" key same for both kloss and normal dataset
                ],
                axis=1,
                out=controls,
            )
            if controls_noise != 0:
                controls = add_timeseries_noise([controls], noise_level=controls_noise, struct_drop=False)[0]

            if multimodal_noise != 0:
                tmp = add_timeseries_noise(
                    [
                        observations["image"],
                        observations["gripper_pos"],
                        observations["gripper_sensors"],
...
def split_trajectories(trajectories: List[TrajectoryNumpy], subsequence_length: int, modalities=None):
    """
    ...
    for traj in trajectories:
        ...
        for offset in (0, subsequence_length // 2):
            ...
            for s, o, c in zip(
                ...
                # through some hoops
                fp.utils.SliceWrapper(fp.utils.SliceWrapper(traj.observations).map(split_fn)),
                fp.utils.SliceWrapper(fp.utils.SliceWrapper(traj.controls).map(split_fn)),
            ):
                # Add to subsequences
...
class SubsequenceDataset(Dataset):
    ...
    def __init__(
        self,
        trajectories: List[TrajectoryNumpy],
        subsequence_length: int,
        modalities=None,
    ):
        ...
        self.subsequences = split_trajectories(trajectories, subsequence_length, modalities)
