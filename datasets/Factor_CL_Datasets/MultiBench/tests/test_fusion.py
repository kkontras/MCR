...
def test_multiplicative_interaction_models(set_seeds):
    ...
    data = [
        ...
        torch.randn((64, 1, 112, 112)),
        torch.cat((torch.ones((32,)), torch.randn((32,))), dim=0).long(),
    ]
    channels = 1
    ...
    fusion = MultiplicativeInteractions2Modal([channels * 8, channels * 32], [channels, channels], "matrix3D")
    out = fusion([encoders[0](data[0]), encoders[1](data[1])])
    assert out.shape == (64, 1, 1)
    assert np.isclose(torch.norm(out).item(), 42.0806884765625)
    assert count_parameters(fusion) == 297

    fusion = MultiplicativeInteractions2Modal([channels * 8, channels * 32], channels * 40, "vector")
    out = fusion([encoders[0](data[0]), encoders[1](data[1])])
    assert out.shape == (64, 32)
    assert np.isclose(torch.norm(out).item(), 98.5400619506836)
    assert count_parameters(fusion) == 576

    fusion = MultiplicativeInteractions2Modal([channels * 8, channels * 32], channels * 40, "scalar", grad_clip=(0.1, 0.2))
    out = fusion([encoders[0](data[0]), encoders[1](data[1])])
    assert out.shape == (64, 32)
    ...
    fusion = MultiplicativeInteractions2Modal([channels * 8], channels * 40, "scalar", grad_clip=(0.1, 0.2))
    out = fusion([encoders[0](data[0])])
    assert np.isclose(torch.norm(out).item(), 27.642284393310547)
    assert count_parameters(fusion) == 18

    fusion = MultiplicativeInteractions3Modal([channels * 8, channels * 32, channels * 8], channels * 40)
    ...
    fusion = MultiplicativeInteractions3Modal([channels * 8, channels * 32, channels * 8], channels * 40, task="affect")
    out = fusion([encoders[0](data[0]), encoders[1](data[1]), encoders[0](data[0])])
...
def test_MULTModel(set_seeds):
    ...
    data = [
        torch.randn([32, 50, 20]),
        torch.randn([32, 50, 5]),
        torch.randn([32, 50, 300]),
        torch.cat((torch.ones((16, 1)), torch.randn((16, 1))), dim=0).long(),
    ]
    hparams = HParams()
