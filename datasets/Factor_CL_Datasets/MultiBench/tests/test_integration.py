...
def test_sl1():
    ...
    from unimodals.common_models import GRU, MLP, Identity, Sequential  # noqa

    data = [
        torch.zeros([32, 50, 35]),
        torch.zeros([32, 50, 74]),
        torch.zeros([32, 50, 300]),
        torch.cat((torch.ones((16, 1)), torch.zeros((16, 1))), dim=0),
    ]
    my_dataset = torch.utils.data.TensorDataset(*data)
    dl = torch.utils.data.DataLoader(my_dataset)

    encoders = [
        Identity().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        Identity().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        Identity().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    ]
    head = Sequential(
...
def test_sl2():
    ...
    from training_structures.gradient_blend import test, train  # noqa
    ...
    data = [
        ...
        torch.zeros([32, 50, 74]),
        torch.zeros([32, 50, 300]),
        torch.cat((torch.ones((16, 1)), torch.zeros((16, 1))), dim=0).long(),
    ]
    my_dataset = torch.utils.data.TensorDataset(*data)
    ...
    # mosi/mosei
    encoders = [
        Transformer(35, 70).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    ...
    unimodal_heads = [
        MLP(70, 32, 2).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        MLP(150, 64, 2).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        MLP(600, 256, 2).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    ]
...
def test_sl3():
    ...
    # mosi/mosei
    encoder = GRU(300, 600, dropout=True, has_padding=False, batch_first=True, last_only=True).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
...
def test_sl4():
    ...
    from fusions.MCTN import Decoder, Encoder  # noqa
    from private_test_scripts.all_in_one import all_in_one_train  # noqa
    from training_structures.MCTN_Level2 import test, train  # noqa
    ...
    dl_faked = [
        [
            [
                torch.zeros((32, 28, 35)),
                torch.zeros((32, 28, 74)),
                torch.zeros((32, 28, 300)),
            ],
            [torch.zeros((32,)), torch.zeros((32,)), torch.zeros((32,))],
            torch.cat((torch.ones((16, 1)), torch.zeros((16, 1))), dim=0).long(),
            torch.cat((torch.ones((16, 1)), torch.zeros((16, 1))), dim=0).long(),
        ]
    ]
...
def test_sl5():
    from training_structures.architecture_search import test, train

    import utils.surrogate as surr

    data = [
        torch.zeros((64, 1, 28, 28)),
        torch.zeros((64, 1, 112, 112)),
        torch.cat((torch.ones((32,)), torch.zeros((32,))), dim=0).long(),
    ]
    my_dataset = torch.utils.data.TensorDataset(*data)
...
def test_sl6():
    ...
    from unimodals.MVAE import TSDecoder, TSEncoder  # noqa
    ...
    encoders = [
        TSEncoder(dim_0, 30, n_latent, timestep, returnvar=False).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        TSEncoder(dim_1, 30, n_latent, timestep, returnvar=False).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        TSEncoder(dim_2, 30, n_latent, timestep, returnvar=False).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    ]

    decoders = [
        TSDecoder(dim_0, 30, n_latent, timestep).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        TSDecoder(dim_1, 30, n_latent, timestep).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
        TSDecoder(dim_2, 30, n_latent, timestep).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
    ]
