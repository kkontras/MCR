...
from unimodals.common_models import GRU, MLP, Linear
...
def test_MutlSimLoss(set_seeds):
    ...
    assert (
        loss(
            torch.zeros((10, 10)),
            torch.cat([torch.zeros((5, 10)), torch.ones((5, 10))], dim=0),
        ).shape
        == ()
    )
    ...
    assert (
        loss(
            ...
            torch.cat([torch.zeros((1, 10)), torch.ones((9, 10))], dim=0),
        ).item()
        == 0
    )
...
def test_MFM_objective(set_seeds):
    ...
    assert np.isclose(
        objective1(
            torch.rand((10, 10)).to(device),
            torch.randint(low=0, high=10, size=(10,)).to(device),
            args,
        ).item(),
        7.1342926025390625,
    )
...
def test_MVAE_objective(set_seeds):
    ...
    args["decoders"] = decoders
    args["reps"] = [
        (torch.randn((10, 10)).to(device), torch.randn((10, 10)).to(device)),
        (torch.randn((10, 10)).to(device), torch.randn((10, 10)).to(device)),
    ]
...
def test_RMFE_object(set_seeds):
    ...
    args["inputs"] = [torch.rand((10, 10)).to(device), torch.rand((10, 10)).to(device)]
    assert np.isclose(
        objective(torch.rand((10, 10)).to(device), torch.ones([10, 10]).to(device), args).item(),
        0.4805612862110138,
    )
    args["inputs"] = []
    try:
...
def test_Regularization(set_seeds):
    ...
    assert np.isclose(
        re.get_regularization_term(torch.rand((10,)), optim_method="min_ent").item(),
        1.5079478,
    )
    assert np.isclose(
        re.get_regularization_term(torch.rand((10,)), optim_method="max_ent_minus").item(),
        -1.138665,
    )
    try:
...
def test_RegularizationLoss(set_seeds):
    encoders = [
        GRU(10, 5, dropout=True, has_padding=True, batch_first=True).to(device),
        GRU(10, 5, dropout=True, has_padding=True, batch_first=True).to(device),
    ]
    fusion = Concat().to(device)
