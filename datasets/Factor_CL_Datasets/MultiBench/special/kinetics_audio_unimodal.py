...
model = torch.nn.Sequential(r50, MLP(1000, 200, 64), torch.nn.Linear(64, num_classes)).cuda(device)
...
def train(ep=0):
    ...
    for fid in range(num_train_loaders):
        ...
        if dataset_size == "small":
            ...
            datas = torch.load("/home/pliang/yiwei/kinetics_medium/train/batch_medium" + str(fid) + ".pdt")
        datas = [d for d in datas if d[1].shape[1] == 763]
        train_dataloader = DataLoader(datas, shuffle=True, batch_size=batch_size)
...
for ep in tqdm(range(epochs)):
    ...
    with torch.no_grad():
        for fid in range(num_valid_loaders):
            if dataset_size == "small":
                datas = torch.load("/home/pliang/yiwei/kinetics_small/valid/batch_37%d.pdt" % fid)
            else:
                datas = torch.load("/home/pliang/yiwei/kinetics_medium/valid/batch_medium%d.pdt" % fid)
            valid_dataloader = DataLoader(datas, shuffle=False, batch_size=batch_size, num_workers=num_workers)
...
for fid in range(num_test_dataloaders):
    if dataset_size == "small":
        ...
        datas = torch.load("/home/pliang/yiwei/kinetics_medium/test/batch_medium%d.pdt" % fid)
