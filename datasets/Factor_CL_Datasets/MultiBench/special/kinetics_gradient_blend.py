    ...
    with torch.no_grad():
        for fid in range(22):
            ...
            for j in train_dataloader:
                total += len(j[0])
                train_x = [
                    j[0].float().cuda(device),
                    j[1].unsqueeze(1).float().cuda(device),
                ]
                train_y = j[-1].cuda(device)
...
for ep in tqdm(range(num_epoch // gb_epoch)):
    ...
    for jj in range(gb_epoch):
        ...
        for fid in range(22):
            ...
            for j in train_dataloader:
                ...
                blendloss = criterion(multimodal_classification_head(catout, training=True), train_y) * weights[-1]
                for ii in range(len(unimodal_models)):
                    loss = criterion(unimodal_classification_heads[ii](outs[ii]), train_y)
                    blendloss += loss * weights[ii]
                totalloss += blendloss * len(j[0])
                blendloss.backward()
                optim.step()
                total += len(j[0])
        print("epoch " + str(jj + ep * gb_epoch) + " blend train loss: " + str(totalloss / total))
    ...
    ftt_dataloader = DataLoader(finetunetrains, shuffle=True, num_workers=num_workers, batch_size=batch_size)
    for jj in range(finetune_epoch):
        ...
        for j in ftt_dataloader:
            ...
            train_y = j[-1].cuda(device)
            blendloss = criterion(finetunehead(fusehead(train_x, training=True), training=True), train_y)
            totalloss += blendloss * len(j[0])
            blendloss.backward()
            optimi.step()
        ...
        with torch.no_grad():
            ...
            print("epoch " + str((ep + 1) * gb_epoch - 1) + " valid loss: " + str(totalloss / total) + (" acc: " + str(float(corrects) / total)))
            if valoss < bestvaloss:
                print("Saving best")
                bestvaloss = valoss
                torch.save(
                    completeModule(unimodal_models, fusehead, finetunehead),
                    "best_kgrb.pt",
                )
