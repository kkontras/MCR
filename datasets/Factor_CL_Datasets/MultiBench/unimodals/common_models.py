...
class GRU(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        if self.has_padding:
            x = pack_padded_sequence(x[0], x[1], batch_first=self.batch_first, enforce_sorted=False)
            out = self.gru(x)[1][-1]
        elif self.last_only:
            out = self.gru(x)[1][0]
        ...
        if dropout:
            ...
            for feat in feats_list:
                if isinstance(feat, nn.Conv2d):
                    pruned_feat = nn.Conv2d(
                        (int(feat.in_channels * prune_factor) if feat.in_channels != 3 else 3),
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute([2, 0, 1])
        x_all = self.transformer(x)
        return x_all[-1], x_all.permute([1, 0, 2])
