...
from .layers import conv2d, crop_like, deconv, predict_flow
...
class OpticalFlowDecoder(nn.Module):
    ...
    def __init__(self, z_dim, initailize_weights=True):
        ...
        self.upsampled_optical_flow6_to_5 = nn.ConvTranspose2d(2, 2, 4, 2, 1, bias=False)
        self.upsampled_optical_flow5_to_4 = nn.ConvTranspose2d(2, 2, 4, 2, 1, bias=False)
        self.upsampled_optical_flow4_to_3 = nn.ConvTranspose2d(2, 2, 4, 2, 1, bias=False)
        self.upsampled_optical_flow3_to_2 = nn.ConvTranspose2d(2, 2, 4, 2, 1, bias=False)

        self.predict_optical_flow2_mask = nn.Conv2d(66, 1, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, tiled_feat, img_out_convs):  # pragma: no cover
        ...
        optical_flow6_up = crop_like(self.upsampled_optical_flow6_to_5(optical_flow6), out_img_conv5)
        out_img_deconv5 = crop_like(self.img_deconv5(optical_flow_in_feat), out_img_conv5)

        concat5 = torch.cat((out_img_conv5, out_img_deconv5, optical_flow6_up), 1)
        optical_flow5 = self.predict_optical_flow5(concat5)
        optical_flow5_up = crop_like(self.upsampled_optical_flow5_to_4(optical_flow5), out_img_conv4)
        ...
        optical_flow4_up = crop_like(self.upsampled_optical_flow4_to_3(optical_flow4), out_img_conv3)
        ...
        optical_flow3_up = crop_like(self.upsampled_optical_flow3_to_2(optical_flow3), out_img_conv2)
