...
def train_surrogate(
    ...
    err = surr.train_simple_surrogate(surrogate, surrogate_criterion, surrogate_optimizer, s_data, ep, device)
    ...
    if not previous_top_k_configurations:
        ...
        for unfolded_conf in unfolded_configurations:
            ...
            if layer == 0:
                ...
                raise ValueError("merge_unfolded_with_sampled: Something weird is happening. previous_top_k_configurations is None, but layer != 0")
...
def compute_temperature(iteration, init, final, decay):
    ...
    """
    temp = (init - final) * np.exp(-((iteration + 1.0) ** 2) / decay**2) + final
    return temp
