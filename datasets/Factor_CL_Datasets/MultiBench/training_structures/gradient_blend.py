...
def gb_estimate(
    unimodal_models,
    multimodal_classification_head,
    fuse,
...
def train(
    ...
    def _trainprocess():
        ...
        tv_dataloader = DataLoader(v_data, shuffle=False, num_workers=8, batch_size=train_dataloader.batch_size)
        ...
        for i in range(num_epoch // gb_epoch):
            ...
            for jj in range(gb_epoch):
                ...
                for j in train_dataloader:
                    ...
                    for ii in range(len(unimodal_models)):
                        loss = criterion(
                            unimodal_classification_heads[ii](outs[ii]),
                            train_y.squeeze(),
                        )
            ...
            for jj in range(finetune_epoch):
                ...
                for j in ftt_dataloader:
                    ...
                    finetunehead.train()
                    fusehead.train()
                    blendloss = criterion(finetunehead(fusehead(train_x)), train_y.squeeze())
                    totalloss += blendloss * len(j[0])
                    blendloss.backward()
                    optimi.step()
    ...
    if track_complexity:
        all_in_one_train(
            _trainprocess,
            unimodal_models + [multimodal_classification_head, fuse] + unimodal_classification_heads,
        )
    else:
        _trainprocess()
