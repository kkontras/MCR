...
import torch
from eval_scripts.complexity import all_in_one_test, all_in_one_train
from eval_scripts.performance import AUPRC, accuracy, eval_affect, f1_score
from eval_scripts.robustness import effective_robustness, relative_robustness, single_plot
    ...
    def _trainprocess():
        ...
        for epoch in range(total_epochs):
            ...
            if task == "classification":
                ...
                print("Epoch " + str(epoch) + " valid loss: " + str(valloss) + " f1_micro: " + str(f1_micro) + " f1_macro: " + str(f1_macro))
                if f1_macro > bestf1:
                    patience = 0
                    bestf1 = f1_macro
                    print("Saving Best")
    ...
    with torch.no_grad():
        ...
        for j in test_dataloader:
            ...
            if is_packed:
                out = model(
                    [
                        [_processinput(i).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for i in j[0]],
                        j[1],
                    ]
                )
            else:
                out = model([_processinput(i).float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) for i in j[:-1]])
            if type(criterion) == torch.nn.modules.loss.BCEWithLogitsLoss or type(criterion) == torch.nn.MSELoss:
                loss = criterion(
                    out,
                    j[-1].float().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
            ...
            #     loss=criterion(out, j[-1].long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")))
                ...
                loss = criterion(
                    out,
                    truth1.long().to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),
                )
            else:
                loss = criterion(
                    out,
        ...
        if task == "classification":
            ...
            print(" f1_micro: " + str(f1_score(true, pred, average="micro")) + " f1_macro: " + str(f1_score(true, pred, average="macro")))
            return {
...
def test(
    ...
    for noisy_modality, test_dataloaders in test_dataloaders_all.items():
        ...
        for measure, robustness_result in robustness_curve.items():
            ...
            print(
                "relative robustness ({}, {}): {}".format(
                    noisy_modality,
                    measure,
                    str(relative_robustness(robustness_result, robustness_key)),
                )
            )
            if len(robustness_curve) != 1:
                robustness_key = "{} {}".format(robustness_key, measure)
            print(
                "effective robustness ({}, {}): {}".format(
                    noisy_modality,
                    measure,
                    str(effective_robustness(robustness_result, robustness_key)),
                )
            )
