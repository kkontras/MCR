...
from fusions.MCTN import L2_MCTN, Seq2Seq
...
def train(
    ...
    for ep in range(epoch):
        ...
        for i, inputs in enumerate(traindata):
            src, trg0, trg1, labels, f_dim = _process_input_L2(inputs, max_seq_len)
        ...
        print("Train Epoch {}, total loss: {}, regression loss: {}, embedding loss: {}".format(ep, sum_total_loss, sum_reg_loss, sum_total_loss - sum_reg_loss))
        ...
        with torch.no_grad():
            for i, inputs in enumerate(validdata):
                # process input
                src, trg0, trg1, labels, feature_dim = _process_input_L2(inputs, max_seq_len)
